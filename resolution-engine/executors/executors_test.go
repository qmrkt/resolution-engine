...
func TestAskCreatorExecutorResponded(t *testing.T) {
	var (
		mu        sync.Mutex
		judgments = map[string]humanJudgmentRecord{}
	)
	const runID = "run-ask-creator"
	const judgmentID = "80:run-ask-creator:ask_creator_node"
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPost:
			...
			// Verify it only allows "creator"
			if len(body.AllowedResponders) != 1 || body.AllowedResponders[0] != "creator" {
				http.Error(w, "ask_creator must only allow creator", 400)
				return
			}
			judgment := humanJudgmentRecord{
				JudgmentID:        body.JudgmentID,
	...
	exec := NewAskCreatorExecutor(server.URL)
	exec.SetPollInterval(20 * time.Millisecond)

	// Simulate creator response after a short delay
	go func() {
		time.Sleep(80 * time.Millisecond)
		mu.Lock()
		j := judgments[judgmentID]
		j.Status = "responded"
		j.ResponseOutcome = 2
		j.ResponseReason = "Creator determined outcome 2"
		j.ResponderAddress = "CREATORADDR"
		j.ResponderRole = "creator"
		j.ResponseHash = "creatorhash"
		j.ResponseSubmittedAt = time.Now().UnixMilli()
		judgments[judgmentID] = j
		mu.Unlock()
	}()
	...
	result, err := exec.Execute(context.Background(), dag.NodeDef{
		...
		Type: "ask_creator",
		Config: map[string]interface{}{
			"prompt":          "Resolve this disputed market.",
			"timeout_seconds": 5,
		},
	}, dag.NewContext(map[string]string{"market_app_id": "80", "resolution_run_id": runID}))
	if err != nil {
		t.Fatal(err)
	}
	if result.Outputs["status"] != "responded" {
	...
	if result.Outputs["dispute_action"] != "creator_ruling" {
		t.Fatalf("expected dispute_action=creator_ruling, got %q", result.Outputs["dispute_action"])
	}
}

// ---------------------------------------------------------------------------
// ask_market_admin tests
// ---------------------------------------------------------------------------

func TestAskMarketAdminExecutorResponded(t *testing.T) {
	var (
		mu        sync.Mutex
		judgments = map[string]humanJudgmentRecord{}
	)
	const runID = "run-ask-admin"
	const judgmentID = "81:run-ask-admin:ask_admin_node"
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPost:
			...
			// Verify it only allows "market_admin"
			if len(body.AllowedResponders) != 1 || body.AllowedResponders[0] != "market_admin" {
				http.Error(w, "ask_market_admin must only allow market_admin", 400)
				return
			}
			judgment := humanJudgmentRecord{
				JudgmentID:        body.JudgmentID,
	...
	exec := NewAskMarketAdminExecutor(server.URL)
	exec.SetPollInterval(20 * time.Millisecond)
	...
	go func() {
		...
		j.Status = "responded"
		j.ResponseOutcome = 0
		j.ResponseReason = "Admin fallback ruling"
		j.ResponderAddress = "ADMINADDR"
		j.ResponderRole = "market_admin"
		j.ResponseHash = "adminhash"
		j.ResponseSubmittedAt = time.Now().UnixMilli()
		judgments[judgmentID] = j
		mu.Unlock()
	}()
	...
	result, err := exec.Execute(context.Background(), dag.NodeDef{
		ID:   "ask_admin_node",
		Type: "ask_market_admin",
		Config: map[string]interface{}{
			"prompt":          "Admin fallback: resolve this disputed market.",
			"timeout_seconds": 5,
		},
	}, dag.NewContext(map[string]string{"market_app_id": "81", "resolution_run_id": runID}))
	if err != nil {
		t.Fatal(err)
	}
	if result.Outputs["status"] != "responded" {
	...
	if result.Outputs["responder_role"] != "market_admin" {
		t.Fatalf("expected responder_role=market_admin, got %q", result.Outputs["responder_role"])
	}
	if result.Outputs["dispute_action"] != "admin_fallback_ruling" {
		t.Fatalf("expected dispute_action=admin_fallback_ruling, got %q", result.Outputs["dispute_action"])
	}
}
